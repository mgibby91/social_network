...
export default function useApplicationData() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("http://localhost:8001/api/comments").then((res) => res.json()),
      fetch("http://localhost:8001/api/comments").then((res) => res.json()),
      fetch("http://localhost:8001/api/comments"),
      fetch("/api/posts"),
      fetch("/api/student_stack"),
      fetch("/api/tutor_experiences"),
      fetch("/api/user_profiles"),
      fetch("/api/users"),
      fetch("/api/user_profiles/mentor_points/:id"),
      fetch("/api/user_profiles/student_points/:id"),
    ]).then((all) => {
